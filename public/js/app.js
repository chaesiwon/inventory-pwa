/* ==============================================
   장기재고 소진계획 관리 시스템 - app.js v6
   ============================================== */

// ── API 서버 주소 ─────────────────────────────
// index.html의 window.API_SERVER 값을 읽음
// '' (빈 문자열) = 같은 서버 (로컬 or 통합배포)
// 'https://xxx.onrender.com' = Render 서버
const API_BASE = (() => {
  const s = (window.API_SERVER || '').trim().replace(/\/$/, '');
  if (s) {
    console.log('✅ API 서버:', s);
    return s;
  }
  // localhost 가 아닌데 API_SERVER가 비어있으면 경고
  if (location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
    console.error('❌ API_SERVER 미설정! index.html에 Render URL을 입력하세요.');
    // 로그인 화면에 경고 표시 (DOM 로드 후)
...
// ── 인증 저장소 ──────────────────────────────
function getAuth()         { try { return JSON.parse(localStorage.getItem('_inv_auth') || 'null'); } catch { return null; } }
function setAuth(u, token) { try { localStorage.setItem('_inv_auth', JSON.stringify({...u, token})); } catch {} }
function clearAuth()       { try { localStorage.removeItem('_inv_auth'); } catch {} }
...
async function api(path, opts = {}, _retry = 0) {
  ...
  try {
    ...
    if (_retry === 0) {
      ...
      if (st) st.textContent = '⏳ 서버 연결 중... (최대 30초 소요)';
      toast('서버 연결 중입니다. 잠시 기다려 주세요...', 'inf');
      await new Promise(r => setTimeout(r, 8000));
      return api(path, opts, 1);
    }
    throw new Error('서버에 연결할 수 없습니다. Render 서버 URL을 확인하세요.');
  }

  const st = document.getElementById('api-status');
  if (st) st.textContent = '';

  // 응답을 텍스트로 먼저 읽기 (빈 응답 대비)
  ...
  catch { throw new Error('응답 파싱 오류: ' + text.slice(0, 100)); }
}

// ── 파일 업로드 fetch (FormData 전용) ────────
// ★ FormData 업로드 시 Content-Type 헤더 직접 설정 금지
//   브라우저가 boundary 포함해서 자동 설정함
async function uploadFetch(path, formData, _retry=0) {
  // ★ FormData 업로드 시 Content-Type 절대 직접 설정 금지
  //   브라우저가 boundary 포함해서 자동 설정함
  const url = API_BASE + '/api' + path;
  const headers = authHeaders();

  console.log('[uploadFetch] 요청 시작:', url, 'auth:', JSON.stringify(headers));

  let res;
  try {
    res = await fetch(url, { method: 'POST', body: formData, headers });
  } catch (networkErr) {
    console.error('[uploadFetch] 네트워크 오류:', networkErr);
    // cold start 재시도 (1회)
    if (_retry === 0) {
      toast('⏳ 서버 연결 중... (최대 30초 소요)', 'inf');
      await new Promise(r => setTimeout(r, 8000));
      return uploadFetch(path, formData, 1);
    }
    throw new Error('서버에 연결할 수 없습니다. 잠시 후 다시 시도하세요.');
  }

  console.log('[uploadFetch] HTTP 응답:', res.status, res.statusText);

  // 응답 본문을 텍스트로 읽기 (빈 응답 대비)
  let text = '';
  try { text = await res.text(); } catch(e) { text = ''; }

  console.log('[uploadFetch] 응답 본문 길이:', text.length, '내용(앞100):', text.slice(0,100));

  // 빈 응답 처리
  if (!text || text.trim() === '') {
    if (res.status === 401) {
      throw new Error('로그인이 만료되었습니다. 새로고침 후 다시 로그인하세요.');
    }
    if (res.status === 0 || _retry === 0) {
      // cold start로 인한 빈 응답 → 재시도
      toast('⏳ 서버 응답 대기 중... (재시도)', 'inf');
      await new Promise(r => setTimeout(r, 5000));
      return uploadFetch(path, formData, _retry + 1);
    }
    throw new Error(`서버 응답 없음 (HTTP ${res.status}). Render Logs 탭에서 오류를 확인하세요.`);
  }
  ...
  try {
    ...
    console.error('[uploadFetch] JSON 파싱 실패. 응답:', text.slice(0, 300));
    // HTML 응답이면 (예: 502 Bad Gateway 페이지)
    if (text.trim().startsWith('<')) {
      throw new Error(`서버 오류 (HTTP ${res.status}). Render 서버가 시작 중이거나 재배포 중일 수 있습니다.`);
    }
    throw new Error('서버가 잘못된 응답을 반환했습니다: ' + text.slice(0, 100));
  }

  if (!res.ok) {
    throw new Error(d.detail || d.error || `HTTP ${res.status} 오류`);
  }
  return d;
}
...
function dlUrl(path) {
  ...
  const tok = auth ? `${sep}_uid=${auth.id}&_tok=${auth.token || ''}` : '';
...
async function loadRefDates() {
  try {
    ...
    ['dash-ref','top20-ref','inv-ref','noplan-ref','plan-ref','cmp-ref','bulk-inv-ref'].forEach(id => {
...
const App = {
  ...
  async init() {
    ...
    try {
      ...
      if (d.logged_in && d.user) { this.showApp(d.user); return; }
    } catch {}
  ...
  showApp(user) {
    ...
    document.querySelectorAll('.sidebar-menu li[data-page]').forEach(li => {
      li.onclick = () => {
        if (li.dataset.page === 'users' && user.role !== 'admin') { toast('관리자만 접근 가능합니다.', 'err'); return; }
  ...
  async login() {
    ...
    if (!API_BASE && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') {
      errEl.textContent = '⚠️ API 서버 URL이 설정되지 않았습니다. index.html의 window.API_SERVER에 Render URL을 입력하세요.';
      return;
    }
  ...
  bindAll() {
    ...
    document.addEventListener('keydown', e => { if (e.key === 'Escape') { Modal.close(); AModal.close(); Users.closeModal(); } });

    // 파일 업로드 영역
    const upFile = document.getElementById('up-file');
    document.getElementById('upload-zone').addEventListener('click', () => upFile.click());
    document.getElementById('upload-zone').addEventListener('dragover', e => e.preventDefault());
    document.getElementById('upload-zone').addEventListener('drop', e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) Upload.do(f); });
    ...
    document.getElementById('bulk-clear-sel-btn').addEventListener('click',  () => Plans.clearBulkSelect());
    document.getElementById('bulk-inv-ref').addEventListener('change', () => { Plans._resetBulkState(); Plans.loadBulkInventory(); });
    ['bulk-inv-lot','bulk-inv-cc','bulk-inv-iname'].forEach(id => {
      const el = document.getElementById(id);
      el?.addEventListener('input', () => { clearTimeout(el._t); el._t = setTimeout(() => { Plans._resetBulkState(); Plans.loadBulkInventory(); }, 400); });
    });
    ...
    ['plan-ref','plan-fac','plan-dept','plan-type'].forEach(id => document.getElementById(id)?.addEventListener('change', () => Plans.loadRegistered()));
    ...
    ['cmp-ref','cmp-fac','cmp-dept'].forEach(id => document.getElementById(id)?.addEventListener('change', () => Compare.search()));
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        btn.closest('.page, body')?.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
        ...
        if (tab.startsWith('cmp-')) {
          ...
          ['noplan','registered','bulk'].forEach(t => document.getElementById('tab-'+t)?.classList.toggle('hidden', tab !== t));
          Plans._resetBulkState();
          Plans.currentTab = tab;
          if (tab === 'noplan')     Plans.loadNoPlan();
          if (tab === 'registered') Plans.loadRegistered();
          if (tab === 'bulk')       Plans.loadBulkInventory();
        }
      });
    });
  },
...
const Dashboard = {
  ...
  async load() {
    ...
    try {
      ...
      this.renderPlanWt(pw.trend || []);
      this.renderCC(cc.items || []);
      this.loadTop20();
      this.loadCompare();
  ...
  renderTrend(t) {
    this._ch('chart-trend', { type:'line', data: {
      ...
      datasets: [
        { label:`장기재고(${Units.amtLabel()})`, tension:.3, fill:true, borderColor:'#1a56db', backgroundColor:'rgba(26,86,219,.08)', data: t.map(r => Units.fmtAmtRaw(r.total_amount)) },
        { label:`소진금액(${Units.amtLabel()})`, type:'bar', backgroundColor:'rgba(5,150,105,.5)', data: t.map(r => Units.fmtAmtRaw(r.total_consumed)) },
      ]
  ...
  renderPlanWt(t) {
    ...
    this._ch('chart-plan-wt', { type:'bar', data: {
      ...
      datasets: [
        { label:`계획중량(${Units.wtLabel()})`, backgroundColor:'rgba(26,86,219,.65)', data: t.map(r => Units.fmtWtRaw(r.plan_weight_ton)) },
        { label:'계획건수', type:'line', yAxisID:'y2', tension:.3, borderColor:'#d97706', data: t.map(r => r.plan_count) },
      ]
    }, options: { responsive:true, scales: { y:{title:{display:true,text:Units.wtLabel()}}, y2:{position:'right',grid:{drawOnChartArea:false}} } }});
  },
  renderCC(items) {
    document.getElementById('cc-body').innerHTML = items.map(r => `<tr>
      <td>${r.cc_name||'-'}</td><td class="num">${num(r.item_count)}</td>
      <td class="num">${Units.fmtWt(r.total_weight)}</td><td class="num">${Units.fmtAmt(r.total_amount)}</td>
      <td class="num">${Units.fmtAmt(r.consumed_amount)}</td>
      <td class="num">${num(r.plan_count)}</td><td class="num">${num(r.actual_count)}</td>
...
const Inventory = {
  ...
  async load() {
    try {
      const d = await api('/inventory?' + new URLSearchParams({...this.p, page:this.pg, page_size:50}));
      document.getElementById('inv-info').innerHTML = `<b>총 ${num(d.total)}건</b> · 기준일: <b>${rd_(d.ref_date)}</b>`;
...
// ═══════════════════════════════════════════
const Plans = {
  currentTab:'noplan', noplanPg:1, regPg:1, bulkInvPg:1, bulkSel: new Set(),
  ...
  async loadNoPlan() {
    const qs = new URLSearchParams({ ref_date:document.getElementById('noplan-ref').value, factory:document.getElementById('noplan-fac').value, item_name:document.getElementById('noplan-iname').value.trim(), cost_center:document.getElementById('noplan-cc').value.trim(), lot_no:document.getElementById('noplan-lot').value.trim(), page:this.noplanPg, page_size:50 });
    try {
      ...
      document.getElementById('noplan-tbody').innerHTML=(d.items||[]).map(r=>`<tr>
        ...
        <td class="num">${Units.fmtWt(r.weight_ton)}</td><td class="num">${Units.fmtAmt(r.amount)}</td>
        <td>${rd_(r.base_date)}</td>
        <td><span class="badge ${r.is_new?'b-new':'b-old'}">${r.is_new?'신규':'기존'}</span></td>
        <td><button class="btn btn-xs btn-primary" onclick="Modal.open('${r.lot_no}','${(r.item_name||'').replace(/'/g,"\\'")}',${r.amount||0},${r.weight_ton||0})">입력</button></td>
  ...
  async loadRegistered() {
    const qs=new URLSearchParams({ref_date:document.getElementById('plan-ref').value,factory:document.getElementById('plan-fac').value,dept:document.getElementById('plan-dept').value,plan_type:document.getElementById('plan-type').value,lot_no:document.getElementById('plan-lot').value.trim(),cost_center:document.getElementById('plan-cc').value.trim(),page:this.regPg,page_size:50});
    try {
      ...
      document.getElementById('plan-tbody').innerHTML=(d.items||[]).map(r=>`<tr>
        ...
        <td>${r.cc_name||'-'}</td><td>${r.lot_no}</td>
        <td class="num">${Units.fmtWt(r.weight_ton)}</td><td class="num">${Units.fmtAmt(r.amount)}</td>
        <td>${rd_(r.base_date)}</td><td>${r.dept||'-'}</td><td>${r.reason||'-'}</td>
        ...
        <td style="display:flex;gap:3px">
          <button class="btn btn-xs btn-outline" onclick="Modal.open('${r.lot_no}','${(r.item_name||'').replace(/'/g,"\\'")}',${r.amount||0},${r.weight_ton||0})">수정</button>
  ...
  async deletePlan(lot_no) {
    ...
    try { await api('/plans/'+encodeURIComponent(lot_no),{method:'DELETE'}); toast('삭제 완료'); this.loadRegistered(); this.loadNoPlan(); }
    catch(e){toast('삭제 오류: '+e.message,'err');}
  },
  async loadBulkInventory() {
    this._resetBulkState();
    const qs=new URLSearchParams({ref_date:document.getElementById('bulk-inv-ref').value,lot_no:document.getElementById('bulk-inv-lot').value.trim(),cost_center:document.getElementById('bulk-inv-cc').value.trim(),item_name:document.getElementById('bulk-inv-iname').value.trim(),page:this.bulkInvPg,page_size:50});
    try {
      const d=await api('/inventory?'+qs);
      document.getElementById('bulk-inv-tbody').innerHTML=(d.items||[]).map(r=>`<tr>
        <td><input type="checkbox" class="bulk-chk" value="${r.lot_no}" onchange="Plans.toggleBulkItem('${r.lot_no}',this.checked)"></td>
  ...
  toggleBulkItem(lot,checked){ checked?this.bulkSel.add(lot):this.bulkSel.delete(lot); this._syncLots(); },
  _syncLots(){
    const ta=document.getElementById('bulk-lots');
    const ex=ta.value.split('\n').map(s=>s.trim()).filter(s=>s);
    ta.value=[...new Set([...ex,...this.bulkSel])].join('\n');
  },
  selectAllBulk(){ this._resetBulkState(); document.querySelectorAll('.bulk-chk').forEach(cb=>{cb.checked=true;this.bulkSel.add(cb.value);}); this._syncLots(); },
  clearBulkSelect(){ this._resetBulkState(); },
  async bulkSave(){
    const dept=document.getElementById('bulk-dept').value, reason=document.getElementById('bulk-reason').value,
          ptype=document.getElementById('bulk-ptype').value, pdate=document.getElementById('bulk-date').value,
          detail=document.getElementById('bulk-detail').value,
          lots=document.getElementById('bulk-lots').value.split('\n').map(s=>s.trim()).filter(s=>s);
    ...
    for(const lot of lots){
      try{ await api('/plans/'+encodeURIComponent(lot),{method:'POST',body:JSON.stringify({dept,reason,plan_type:ptype,plan_date:pdate,detail_plan:detail})}); ok++; }
      catch(e){ fail++; errors.push(`${lot}: ${e.message}`); }
    }
    res.className=fail===0?'suc':'err';
    res.innerHTML=`${fail===0?'✅':'⚠️'} 완료 — 성공 <b>${ok}건</b>, 실패 <b>${fail}건</b>`+(errors.length?'<br><small>'+errors.slice(0,3).join('<br>')+'</small>':'');
  ...
  downloadTemplate(){ window.open(dlUrl('/plans/export-template'+(document.getElementById('plan-ref').value?'?ref_date='+document.getElementById('plan-ref').value:''))); },
  async excelUpload(file){
    const fd=new FormData(); fd.append('file',file);
    try{
      const d=await uploadFetch('/plans/bulk-upload',fd);
...
const Upload = {
  async do(file) {
    ...
    res.className='inf'; res.textContent='⏳ 서버 연결 확인 중...';
    res.classList.remove('hidden');

    // 1단계: 서버 wake-up ping (cold start 대응)
    try {
      const pingUrl = (API_BASE||'') + '/health';
      const pingRes = await fetch(pingUrl, {method:'GET', signal: AbortSignal.timeout(10000)});
      if (!pingRes.ok) {
        res.textContent='⏳ 서버 시작 중... 잠시 기다려 주세요 (최대 30초)';
        await new Promise(r=>setTimeout(r, 8000));
      } else {
        res.textContent='⏳ 업로드 처리 중...';
      }
    } catch(pingErr) {
      res.textContent='⏳ 서버 연결 중... (최초 접속 시 30초 소요)';
      await new Promise(r=>setTimeout(r, 5000));
    }

    const fd=new FormData(); fd.append('file',file);
    try{
      const d=await uploadFetch('/upload',fd);
      res.className='suc';
  ...
  async loadHist(){
    try{
      ...
      document.getElementById('hist-tbody').innerHTML=(d.history||[]).map(r=>`<tr>
        ...
        <td><button class="btn btn-xs btn-danger" onclick="Upload.deleteOne('${r.upload_id}','${(r.filename||'').replace(/'/g,"\\'")}')">삭제</button></td>
...
const Compare = {
  ...
  init(){ this.search(); },
  search(){ this.pg=1; this.load(); },
  get p(){ return {ref_date:document.getElementById('cmp-ref').value,factory:document.getElementById('cmp-fac').value,dept:document.getElementById('cmp-dept').value}; },
  _ch(id,cfg){ if(this.charts[id]) this.charts[id].destroy(); const el=document.getElementById(id); if(el) this.charts[id]=new Chart(el.getContext('2d'),cfg); },
  async load(){
    try{
      const isSummary=!document.getElementById('tab-cmp-summary').classList.contains('hidden');
      if(isSummary) await this.loadSummary();
      else          await this.loadList();
    }catch(e){toast('비교 오류: '+e.message,'err');}
  },
  async loadSummary(){
    const [s,pw]=await Promise.all([api('/compare/summary?'+new URLSearchParams(this.p)),api('/dashboard/plan-weight-trend')]);
    const pt=s.plan_total||1,ac=s.action_count||0,nc=s.no_action_count||0;
    document.getElementById('cmp-kpi').innerHTML=`
      <div class="kpi c4"><div class="kpi-label">달성률</div><div class="kpi-value">${(ac/pt*100).toFixed(1)}%</div><div class="kpi-sub">조치 ${num(ac)} / 전체 ${num(pt)}</div></div>
      <div class="kpi c3"><div class="kpi-label">미조치 건수</div><div class="kpi-value">${num(nc)}건</div><div class="kpi-sub">${Units.fmtWt(s.no_action_weight)}</div></div>
      <div class="kpi c3" style="border-top-color:#9333ea"><div class="kpi-label">미조치 금액</div><div class="kpi-value">${Units.fmtAmt(s.no_action_amount)}</div></div>
      <div class="kpi c1"><div class="kpi-label">조치 금액</div><div class="kpi-value">${Units.fmtAmt(s.action_amount)}</div><div class="kpi-sub">${Units.fmtWt(s.action_weight)}</div></div>
      <div class="kpi c5"><div class="kpi-label">소진금액</div><div class="kpi-value">${Units.fmtAmt(s.consumed_amount)}</div></div>`;
    const aT=[...new Set([...(s.plan_by_type||[]).map(r=>r.plan_type||'미등록'),...(s.actual_by_type||[]).map(r=>r.actual_type||'기타')])];
    ...
    this._ch('chart-cmp-count',{type:'bar',data:{labels:aT,datasets:[{label:'계획 건수',backgroundColor:'rgba(26,86,219,.7)',data:aT.map(t=>pM[t]?.plan_count||0)},{label:'실적 건수',backgroundColor:'rgba(5,150,105,.7)',data:aT.map(t=>aM[t]?.actual_count||0)}]},options:{responsive:true,plugins:{legend:{position:'top'}}}});
    this._ch('chart-cmp-wt',{type:'bar',data:{labels:aT,datasets:[{label:`계획(${Units.wtLabel()})`,backgroundColor:'rgba(26,86,219,.7)',data:aT.map(t=>Units.fmtWtRaw(pM[t]?.plan_weight||0))},{label:`실적(${Units.wtLabel()})`,backgroundColor:'rgba(5,150,105,.7)',data:aT.map(t=>Units.fmtWtRaw(aM[t]?.actual_weight||0))}]},options:{responsive:true,plugins:{legend:{position:'top'}}}});
    const p=pw.trend||[];
    this._ch('chart-plan-monthly',{type:'bar',data:{labels:p.map(r=>r.plan_month||'-'),datasets:[{label:`계획중량(${Units.wtLabel()})`,backgroundColor:'rgba(26,86,219,.6)',data:p.map(r=>Units.fmtWtRaw(r.plan_weight_ton))}]},options:{responsive:true,scales:{y:{beginAtZero:true}}}});
  },
  async loadList(){
    const d=await api('/compare?'+new URLSearchParams({...this.p,page:this.pg,page_size:50}));
    document.getElementById('cmp-info').innerHTML=`<b>총 ${num(d.total)}건</b>`;
    document.getElementById('cmp-tbody').innerHTML=(d.items||[]).map(r=>{
      const match=r.type_match==null?'<span class="badge b-old">-</span>':r.type_match?'<span class="badge b-blue">일치</span>':'<span class="badge b-mismatch">불일치</span>';
      const act=r.action_status==='조치'?'<span class="badge b-action">조치</span>':'<span class="badge b-noaction">미조치</span>';
      return `<tr>
        <td>${r.factory}</td><td>${r.item_type}</td><td>${r.item_code}</td>
        <td title="${r.item_name}">${r.item_name.slice(0,20)}</td>
        <td>${r.cc_name||r.cost_center||'-'}</td><td>${r.lot_no}</td>
        ...
        <td>${r.plan_type||'-'}</td><td>${r.plan_date||'-'}</td>
        <td>${r.actual_type_manual||r.actual_type||'-'}</td><td>${dt_(r.process_date)}</td>
        <td>${match}</td><td>${act}</td>
        <td>${r.actual_id?`<button class="btn btn-xs btn-outline" onclick="AModal.open(${r.actual_id},'${r.actual_type_manual||r.actual_type||'기타'}')">수정</button>`:'-'}</td>
      </tr>`;
...
const Modal = {
  ...
  async open(lot_no,item_name,amount,wt){
    this.lot=lot_no;
    document.getElementById('modal-info').innerHTML=`<b>LOT NO:</b> ${lot_no}<br><b>품명:</b> ${item_name}<br><b>금액:</b> ${Units.fmtAmt(amount)} &nbsp; <b>중량:</b> ${Units.fmtWt(wt)}`;
    try{
  ...
  async save(){
    const body={dept:document.getElementById('f-dept').value,reason:document.getElementById('f-reason').value,plan_type:document.getElementById('f-ptype').value,plan_date:document.getElementById('f-date').value,detail_plan:document.getElementById('f-detail').value};
...
const AModal = {
  ...
  async save(){
    try{ await api('/actuals/'+this.id+'/type',{method:'PATCH',body:JSON.stringify({actual_type_manual:document.getElementById('af-type').value})}); toast('수정 완료!'); this.close(); Compare.search(); }
...
const Users = {
    ...
    try{
      ...
      document.getElementById('users-tbody').innerHTML=(d.users||[]).map(u=>`<tr>
        ...
        <td style="display:flex;gap:3px">
          ...
          <button class="btn btn-xs ${u.is_active?'btn-danger':'btn-outline'}" onclick="Users.toggleActive(${u.id},${u.is_active})">${u.is_active?'비활성화':'활성화'}</button>
        </td>
  ...
  openCreateModal(){
    ...
    document.getElementById('u-role').value='user'; document.getElementById('u-dept').value=''; document.getElementById('u-active').value='1';
  ...
  openEditModal(u){
    ...
    document.getElementById('u-role').value=u.role||'user'; document.getElementById('u-dept').value=u.department||''; document.getElementById('u-active').value=u.is_active??1;
    document.getElementById('umodal').classList.remove('hidden');
  },
  closeModal(){ document.getElementById('umodal').classList.add('hidden'); },
  async save(){
    const uid=document.getElementById('u-id').value, isEdit=!!uid;
    const body={display_name:document.getElementById('u-dname').value,role:document.getElementById('u-role').value,department:document.getElementById('u-dept').value,is_active:parseInt(document.getElementById('u-active').value)};
    const pw=document.getElementById('u-pw').value; if(pw) body.password=pw;
    try{
      if(isEdit){ await api('/users/'+uid,{method:'PUT',body:JSON.stringify(body)}); toast('수정 완료'); }
      else{
        const username=document.getElementById('u-username').value.trim(), dname=document.getElementById('u-dname').value.trim();
        if(!username){toast('아이디를 입력하세요.','err');return;}
        if(!pw){toast('비밀번호를 입력하세요.','err');return;}
        if(!dname){toast('이름을 입력하세요.','err');return;}
        await api('/users',{method:'POST',body:JSON.stringify({...body,username,password:pw,display_name:dname})}); toast('사용자 추가 완료');
      }
  ...
  async toggleActive(id,cur){
    const ns=cur?0:1;
    if(!confirm(`사용자를 ${ns?'활성화':'비활성화'}하시겠습니까?`)) return;
    try{ await api('/users/'+id,{method:'PUT',body:JSON.stringify({is_active:ns})}); toast('변경 완료'); this.load(); }
